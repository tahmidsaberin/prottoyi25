...
export default async (req) => {
  ...
  try {
    ...
    const course      = formData.get("course") || "general";
    ...
    await store.set(safeKey, buffer, {
      metadata: {
    ...
    return new Response(JSON.stringify({ success: true, key: safeKey, name: file.name }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err) {
    console.error("courses-upload error:", err);
